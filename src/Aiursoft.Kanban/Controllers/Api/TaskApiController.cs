...
public sealed class TaskApiController(
    ...
    IAuthorizationService authorizationService) : ControllerBase
{
    ...
    public async Task<IActionResult> Mine(
        ...
        [FromQuery] string status = "incomplete",
        [FromQuery] string? labelIds = null,
        [FromQuery] string labelMode = "any",
        [FromQuery] string sort = "planned-end-desc")
    {
        var currentUserId = CurrentUserId();
        var queryUserId = string.IsNullOrWhiteSpace(targetUserId) ? currentUserId : targetUserId.Trim();
        var canViewAnyUserTasks = (await authorizationService.AuthorizeAsync(
            User,
            AppPermissionNames.CanViewAnyUserTasks)).Succeeded;
        if (queryUserId != currentUserId && !canViewAnyUserTasks)
        {
            return this.Protocol(Code.Unauthorized, "You cannot view another user's tasks.");
        }

        var targetUser = await userManager.FindByIdAsync(queryUserId);
        if (targetUser == null)
        {
            return this.Protocol(Code.NotFound, "Target user not found.");
        }
        ...
        var availableLabels = statusFilteredCards
            ...
            .Select(group => new TaskLabelFilterDto
            {
                Id = group.Key,
        ...
        if (selectedLabelIds.Count > 0)
        {
            filteredCards = normalizedLabelMode == "all"
                ? filteredCards.Where(card => selectedLabelIds.All(
                    labelId => card.CardLabels.Any(link => link.LabelId == labelId)))
                : filteredCards.Where(card => card.CardLabels.Any(
                    link => selectedLabelIds.Contains(link.LabelId)));
        }

        var availableUsers = canViewAnyUserTasks
            ? (await db.Users
                .OrderBy(user => user.DisplayName)
                .ThenBy(user => user.UserName)
                .ToListAsync())
                .Select(MobileApiMapper.ToUserDto)
                .ToList()
            : [MobileApiMapper.ToUserDto(targetUser)];

        return this.Protocol(new MyTasksResponse
        {
            Code = Code.ResultShown,
            Message = "Assigned tasks.",
            Cards = ApplySort(filteredCards, normalizedSort).Select(MobileApiMapper.ToTaskDto).ToList(),
            TargetUser = MobileApiMapper.ToUserDto(targetUser),
            IsViewingOtherUser = queryUserId != currentUserId,
            CanViewAnyUserTasks = canViewAnyUserTasks,
            AvailableUsers = availableUsers,
            AvailableLabels = availableLabels,
            SelectedLabelIds = selectedLabelIds.Order().ToList(),
    ...
    private static string NormalizeLabelMode(string? labelMode) =>
        labelMode?.Trim().ToLowerInvariant() == "all" ? "all" : "any";
